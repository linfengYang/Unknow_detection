...
from Dataset.load_nilm_data import load_from_csv
...
if __name__ == '__main__':
    ...
    for problem in os.listdir(config['data_path']):  # for loop on the all datasets in "data_dir" directory
        ...
        for i in unknows_Plaid2017_list:
            ...
            Data=Data_Loader(config,X_train,y_train,X_test,y_test,images_train_per,images_test_data)
            ...
            train_unknow(best_model,train_loader,val_loader,test_loader,device)
            # unknow_module(i,X_train,y_train,X_test,y_test)
            unknow_module(i,cnt)
            exit()
            '''
