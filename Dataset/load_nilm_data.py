...
def load_whited_from_csv(dirs):
    ...
    name_dict={'Charger':0,'Fan':1, 'GameConsole':2,'HairDryer':3,'Kettle':4,'LEDLight':5,'LightBulb':6,'Microwave':7,'Mixer':8,'PowerSupply':9,'Toaster':10,'TV':11}
    ...
    return data,labels
def load_whited_totle_data(image_dirs,dirs=None):
    image_path=[]
    labels=[]
    data=[]
    # name_dict={'Air Conditioner':0,'Compact Fluorescent Lamp':1,'Fan':2,'Fridge':3,'Hairdryer':4,'Heater':5,'Incandescent Light Bulb':6,'Laptop':7,'Microwave':8,'Vacuum':9,'Washing Machine':10}
